...
export async function onRequestPost(context) {
  ...
  if (p.direction === 'I_OWE' && Number(p.counterparty_user_id) !== Number(user.id)) return error(403, 'Solo la contraparte puede confirmar')

  await DB.prepare(`
    UPDATE payments
    SET confirmation_status='CONFIRMED', confirmed_by_user_id=?, confirmed_at=datetime('now'), confirmation_note=?
    WHERE id=?
